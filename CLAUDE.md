...
Skills that need third-party libraries declare them in `requirements.txt` (kept lean). Install with `pip install -r requirements.txt` inside the activated `.venv`. Currently: `pyshp` (shapefile reading in the `synthesize-*` location skills) and `PyYAML` (config reading in the `synthesize-*` skills, incl. `synthesize-sales`).
