...
Implements docs/search/Search-events.md. Live step (not reproducible):
...
The TAVILY_API_KEY is read from the environment or the repo-root .env (never logged).
"""
...
def find_repo_root(start: Path) -> Path:
    for p in [start, *start.parents]:
        if (p / "docs" / "search" / "Search-events.md").exists():
            return p
    raise SystemExit("Could not locate repo root (docs/search/Search-events.md not found).")
...
def load_api_key(repo_root: Path) -> str:
    """TAVILY_API_KEY from the environment, else parsed from the repo-root .env."""
        ...
        if env.exists():
            for line in env.read_text(encoding="utf-8").splitlines():
                ...
                if k.strip() == "TAVILY_API_KEY":
                    ...
                    break
    if not key:
        raise SystemExit("TAVILY_API_KEY not found in the environment or .env — cannot search.")
    return key
