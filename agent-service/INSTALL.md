...
- `GOTOAI_AGENT_API_KEY` — bearer key required by the web API's `/v1` endpoints (the CLI
  doesn't use it). Leave blank to disable auth (dev only), or generate one with
  `python -c "import secrets; print('agt_' + secrets.token_urlsafe(32))"`. It must match the
  `GOTOAI_AGENT_API_KEY` set in `web-app/.env`.
