...
class GemmaLLM:
    ...
    def generate_tools(self, messages: list[Message], tool_schemas: list[dict], *,
                       max_new_tokens: int | None = None,
                       stop_ids: tuple[int, ...] = (1, 49, 106)) -> str:
        """One tool-aware generation step for the agentic loop (agent.analyst).

        Applies the chat template with ``tools=``, generates greedily (a 4B model emits
        tool-call syntax more reliably), and decodes **keeping special tokens** so the
        tool-call markers survive for parsing. Stops on Gemma's eos/<tool_call|>/<turn|>.
        """
        self.load()
        inputs = self.processor.apply_chat_template(
        ...
        with torch.inference_mode():
            out = self.model.generate(
                **inputs, max_new_tokens=max_new_tokens or config.MAX_NEW_TOKENS,
                do_sample=False, eos_token_id=list(stop_ids),
                pad_token_id=self.processor.tokenizer.pad_token_id or stop_ids[0],
            )
        return self.processor.tokenizer.decode(out[0][prompt_len:], skip_special_tokens=False)
    ...
    def generate_stream(
        ...
        *,
        ...
        """Yield decoded text pieces as they are generated (for SSE chat).

        Uses transformers' TextIteratorStreamer: generation runs on a worker thread while
        this generator yields the tokens it emits. The caller must still serialise GPU
        access (the web API holds a lock for the stream's duration).
        """
        import threading
