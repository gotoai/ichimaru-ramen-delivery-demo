...
The algorithm below computes the sales for a single (store, date). The skill applies it for **every store** in `DATA/s03_primary/store.tsv` over the date range `time_horizon/sales_history` in `pipeline/config/config.yaml` (start = January 1st of the year two years before the end year; end = the day 2 days before the current system date in JST), producing one output row per (store, date).

#### Sampling and reproducibility
- All randomness draws from a single RNG seeded once with `synthetics/random_seed` (629) in `pipeline/config/config.yaml`, consumed in a fixed order: stores in ascending `store_name`, dates ascending, and within a (store, date) the steps in order — baseline, then each home building, competitor and event (in their `*.tsv` file order), then temperature, then rain.
...
- Processes
  1. Baseline
    ...
    - Sample the sacle Sb, indicated by file `pipeline/config/config.yaml`, `synthetics/sales/baseline/scale_sampling`
    - Sample the noise Nb, indicated by file `pipeline/config/config.yaml`, `synthetics/sales/baseline/noise_sampling`
  ...
  2. Home building influences
    ...
    Where Sh and Nh is the sampled scale and noise indicated by file `pipeline/config/config.yaml`, `synthetics/sales/home_building/[scale|noise]_sampling`
  ...
  3. Competitor influences
    ...
    Where Sc and Nc is the sampled scale and noise indicated by file `pipeline/config/config.yaml`, `synthetics/sales/competitor/[scale|noise]_sampling`
  ...
  4. Event influences
    ...
    Where Se and Ne is the sampled scale and noise indicated by file `pipeline/config/config.yaml`, `synthetics/sales/event/[scale|noise]_sampling`
  ...
  5. Temperature influences
    ...
    Where St and Nt is the sampled scale and noise indicated by file `pipeline/config/config.yaml`, `synthetics/sales/weather_temperature/[scale|noise]_sampling`
  ...
  6. Rain influences
    ...
    Where Sr and Nr are the sampled scale and noise indicated by file `pipeline/config/config.yaml`, `synthetics/sales/weather_rain/[scale|noise]_sampling`
