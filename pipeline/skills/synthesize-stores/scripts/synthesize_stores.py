...
For every prefecture referenced in ``docs/pipeline/profiles/Locations.md`` (with its store
...
Inputs
  * ``docs/pipeline/profiles/Locations.md``            - prefectures and store counts
...
def read_random_seed(config_path: Path) -> int:
    """Read synthetics/random_seed from pipeline/config/config.yaml."""
