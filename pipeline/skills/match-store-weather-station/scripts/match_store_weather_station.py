...
Implements docs/pipeline/synthetics/Match-store-weather-station.md. Reproduces — and
...
def find_repo_root(start: Path) -> Path:
    for p in [start, *start.parents]:
        if (p / "docs" / "pipeline" / "synthetics" / "Match-store-weather-station.md").exists():
            return p
    raise SystemExit(
        "Could not locate repo root (docs/pipeline/synthetics/Match-store-weather-station.md not found).")
