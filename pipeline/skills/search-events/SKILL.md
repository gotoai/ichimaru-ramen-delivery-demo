...
description: >-
  ...
  each Ichimaru location via the Tavily API, per docs/pipeline/search/Search-events.md. Reduces
...
[docs/pipeline/search/Search-events.md](../../../docs/pipeline/search/Search-events.md). **Live step**:
...
- `TAVILY_API_KEY` is read from the environment or `pipeline/.env` (git-ignored)
...
python skills/search-events/scripts/search_events.py --dry-run
