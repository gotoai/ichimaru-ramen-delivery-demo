...
Implements docs/pipeline/demand-forecast/dfm_modeling.md. A LightGBM regression model
...
The random seed comes from pipeline/config/config.yaml: modeling/training/random_seed.
"""
...
# parameters keep their defaults. See docs/pipeline/demand-forecast/dfm_modeling.md.
PARAM_GRID = {
    "max_depth": [3, 5, 7],
...
def find_repo_root(start: Path) -> Path:
    for p in [start, *start.parents]:
        if (p / "docs" / "pipeline" / "demand-forecast" / "dfm_modeling.md").exists():
            return p
    raise SystemExit("Could not locate repo root (docs/pipeline/demand-forecast/dfm_modeling.md not found).")
...
def main() -> int:
    ...
    config = yaml.safe_load((repo / "pipeline" / "config" / "config.yaml").read_text(encoding="utf-8")) or {}
