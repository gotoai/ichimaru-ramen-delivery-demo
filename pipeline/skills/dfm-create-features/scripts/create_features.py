...
Implements docs/pipeline/demand-forecast/dfm_features.md. The row granularity is one row
...
def find_repo_root(start: Path) -> Path:
    for p in [start, *start.parents]:
        if (p / "docs" / "pipeline" / "demand-forecast" / "dfm_features.md").exists():
            return p
    raise SystemExit("Could not locate repo root (docs/pipeline/demand-forecast/dfm_features.md not found).")
