...
Implements docs/pipeline/search/Weather-forecast.md. For each target prefecture it reads
...
def find_repo_root(start: Path) -> Path:
    for p in [start, *start.parents]:
        if (p / "docs" / "pipeline" / "search" / "Weather-forecast.md").exists():
            return p
    raise SystemExit("Could not locate repo root (docs/pipeline/search/Weather-forecast.md not found).")
...
def main() -> int:
    ...
    offices = read_target_offices(repo_root / "docs" / "pipeline" / "profiles" / "Locations.md",
                                  area["offices"])
