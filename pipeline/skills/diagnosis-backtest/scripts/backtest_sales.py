...
Implements docs/pipeline/diagnosis/Backtest.md. Re-scores the trained model on the most
...
predicted vs actual side by side for diagnosis analysis:
  ...
  2. Build the feature matrix following the Inference contract in
     docs/pipeline/demand-forecast/dfm_prediction.md: drop the 3 key columns and the
...
def find_repo_root(start: Path) -> Path:
    for p in [start, *start.parents]:
        if (p / "docs" / "pipeline" / "diagnosis" / "Backtest.md").exists():
            return p
    raise SystemExit("Could not locate repo root (docs/pipeline/diagnosis/Backtest.md not found).")
