...
description: >-
  ...
  synthesize-stores), driven entirely by pipeline/config/config.yaml. Per prefecture in
...
Reproducible via `synthetics/random_seed` in `pipeline/config/config.yaml` (competitors are
...
- **Config-driven ranges.** All counts, baseline/unit ranges, and the business
  duration come from `pipeline/config/config.yaml`; no code change is needed to retune them.
