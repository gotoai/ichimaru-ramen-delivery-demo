...
Implements docs/pipeline/demand-forecast/dfm_explanation.md. Uses LightGBM's native exact
...
def find_repo_root(start: Path) -> Path:
    for p in [start, *start.parents]:
        if (p / "docs" / "pipeline" / "demand-forecast" / "dfm_explanation.md").exists():
            return p
    raise SystemExit("Could not locate repo root (docs/pipeline/demand-forecast/dfm_explanation.md not found).")
